...
T_jupiter = 60*60*24*4332.59 # Período orbital de Júpiter en segundos
a_jupiter = 7.785472e11      # Semieje mayor de la órbita de Júpiter en metros
...
fig = plt.figure()
ax = fig.add_subplot(111, aspect='equal', xlim=(-4.5*a_jupiter, 4.5*a_jupiter), ylim=(-4.5*a_jupiter, 4.5*a_jupiter))
...
jupiter_orbit, = ax.plot([], [], color='orange', linestyle='dotted')
...
jupiter_dot, = ax.plot([], [], 'o', color='orange', markersize=8)
mercury_dot, = ax.plot([], [], 'o', color='purple', markersize=4)
sun_dot, = ax.plot([], [], 'o', color='yellow', markersize=20)
ax.set_title('Órbitas de la Tierra, Marte, la Luna, Júpiter y Mercurio')
...
def init():
    ...
    sun_dot.set_data([], [])
    return earth_orbit, mars_orbit, moon_orbit, jupiter_orbit, mercury_orbit, earth_dot, mars_dot, moon_dot, jupiter_dot, mercury_dot, sun_dot
...
def animate(time):
  ...
  #  x_jupiter, y_jupiter = calculate_position(time, T_jupiter, a_jupiter)
   ...
   # jupiter_orbit.set_data(np.append(jupiter_orbit.get_xdata(), x_jupiter), np.append(jupiter_orbit.get_ydata(), y_jupiter))
    ...
    #jupiter_dot.set_data(x_jupiter, y_jupiter)
    mercury_dot.set_data(x_mercury, y_mercury)
    sun_dot.set_data(0, 0)
    
    return earth_orbit, mars_orbit, moon_orbit,  mercury_orbit, earth_dot, mars_dot, moon_dot, jupiter_dot, mercury_dot, sun_dot#jupiter_orbit,

# Crear la animación
ani = FuncAnimation(fig, animate, init_func=init, frames=np.arange(0, max(T_earth, T_mars, T_moon,  T_mercury), 3600), interval=30, blit=True)#T_jupiter,
